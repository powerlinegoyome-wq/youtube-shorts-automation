...
def safe_main():
    ...
    with open("log_discovery.txt", "w") as f:
        f.write("--- BAŞLADI ---\n")
        
    try:
        # Redirect stdout and stderr to the file as well
        class Tee:
        ...
        log_file = open("log_discovery.txt", "a")
        ...
        try:
            ...
            subprocess.run(["git", "config", "--global", "user.email", "bot@automation.com"])
            subprocess.run(["git", "config", "--global", "user.name", "Automation Bot"])
            subprocess.run(["git", "add", "log_discovery.txt", "flow_requests.json", "flow_best_endpoint.json", "*.png"])
            subprocess.run(["git", "commit", "-m", "Auto-push discovery logs"])
            subprocess.run(["git", "push", "origin", "main"])
            print("✅ Loglar GitHub'a başarıyla gönderildi (log_discovery.txt, json vb.)")
        except Exception as e:
